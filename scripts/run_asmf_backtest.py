"""Run and persist an ASMF point-in-time signal evaluation from SQLite."""
...
def parser() -> argparse.ArgumentParser:
    value = argparse.ArgumentParser(
        description="Run and persist a point-in-time, cost-aware ASMF historical backtest."
    )
...
def main(arguments: list[str] | None = None) -> int:
    ...
    try:
        ...
        result = run_strategy_backtest(
            strategy="ASMF", symbols=symbols, start_date=args.start_date,
        ...
        connection.close()
    print(
        f"[PASS] ASMF point-in-time BACKTEST; run_id={result.run.run_id}; "
        f"symbols={len(result.run.symbols)}; decisions={result.run.config['decision_count']}; "
        f"trades={result.run.trade_count}; actions={result.run.config['action_counts']}"
    )
