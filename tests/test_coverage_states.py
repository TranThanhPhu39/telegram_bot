...
def test_v8_preserves_v7_rows_and_accepts_new_datasets(tmp_path, monkeypatch) -> None:
    ...
    assert migrations.bootstrap_schema(conn) == 10
    ...
    assert migrations.bootstrap_schema(conn) == 10   # idempotent
...
def test_v8_is_atomic_when_a_step_fails(tmp_path, monkeypatch) -> None:
    ...
    assert migrations.bootstrap_schema(conn) == 10   # and recovers cleanly
